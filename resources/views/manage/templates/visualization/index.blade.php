<x-app-layout>
    ...
    <x-slot name="header">
        <h3 class="text-lg leading-6 font-medium text-gray-900">
            {{ __('Visualization templates') }}
        </h3>
        <p class="mt-2 max-w-7xl text-sm text-gray-500">
            {{ __('List of existing visualization templates') }}
        </p>
    </x-slot>
                            ...
                            @forelse($records as $record)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-center text-sm font-medium text-gray-900">
                                        <ul>
                                        @foreach($record->getHumanReadableDataRequirement() as $req)
                                            <li>{{ $req }}</li>
                                        @endforeach
                                        </ul>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('manage.templates.visualization.destroy', $record->id) }}" x-on:click.prevent="confirmThenDelete($el)" class="text-red-600">{{ __('Delete') }}</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
