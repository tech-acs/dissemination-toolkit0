<x-app-layout>
    ...
    <x-slot name="header">
        ...
        <p class="mt-2 max-w-7xl text-sm text-gray-500">
            {{ __('List of existing story templates') }}
        </p>
    </x-slot>
    ...
    <div class="flex flex-col max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-right">
            <a><x-button popovertarget="coming-soon-popover" class="opacity-25">{{ __('Update') }}</x-button></a>
        </div>
                            ...
                            @forelse($records as $record)
                                <tr>
                                    <td class="px-6 py-4 text-sm font-medium text-gray-900">
                                        <img class="border w-72" src="{{$record->thumbnailPath}}">
                                    </td>
                                    <td class="px-6 py-4 text-sm font-medium text-gray-900 flex flex-col justify-start">
                                        <div class="font-semibold underline mb-2">{{$record->name}}</div>
                                        <div>{{$record->description}}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('manage.templates.story.destroy', $record->id) }}" x-on:click.prevent="confirmThenDelete($el)" class="text-red-600">{{ __('Delete') }}</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
